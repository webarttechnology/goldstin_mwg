		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_array( $options ) && ! empty( $options ) ) {
					...
					echo ( ! empty( $this->field['empty_message'] ) ) ? $this->field['empty_message'] : esc_html__( 'No data provided for this option type.', 'woo-gallery-slider' );

				}
			} else {

				echo '<label class="wcgs-checkbox">';
				echo '<input type="hidden" name="' . $this->field_name() . '" value="' . $this->value . '" class="wcgs--input"' . $this->field_attributes() . '/>';
