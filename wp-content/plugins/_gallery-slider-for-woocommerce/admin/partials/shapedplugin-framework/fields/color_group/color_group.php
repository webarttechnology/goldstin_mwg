		...
		public function render() {
			...
			if ( ! empty( $options ) ) {
				foreach ( $options as $key => $option ) {
					...
					echo '<div class="wcgs--title">' . $option . '</div>';
					echo '<input type="text" name="' . $this->field_name( '[' . $key . ']' ) . '" value="' . $color_value . '" class="wcgs-color"' . $default_attr . $this->field_attributes() . '/>';
					echo '</div>';
