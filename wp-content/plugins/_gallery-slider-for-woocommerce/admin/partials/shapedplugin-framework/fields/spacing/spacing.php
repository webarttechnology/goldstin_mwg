 ...
 *
 */
if( ! class_exists( 'WCGS_Field_spacing' ) ) {
  class WCGS_Field_spacing extends WCGS_Fields {
    ...
    public function render() {
      ...
      if( ! empty( $args['all'] ) ) {
        ...
        echo ( ! empty( $args['all_icon'] ) ) ? '<span class="wcgs--label wcgs--label-icon">'. $args['all_icon'] .'</span>' : '';
        echo '<input type="text" name="'. $this->field_name('[all]') .'" value="'. $value['all'] .'"'. $placeholder .' class="wcgs-number" />';
        ...
        foreach( $properties as $property ) {
          ...
          echo '<div class="wcgs--input">';
          echo ( ! empty( $args[$property.'_icon'] ) ) ? '<span class="wcgs--label wcgs--label-icon">'. $args[$property.'_icon'] .'</span>' : '';
          echo '<input type="text" name="'. $this->field_name('['. $property .']') .'" value="'. $value[$property] .'"'. $placeholder .' class="wcgs-number" />';
          echo ( count( $args['units'] ) === 1 && ! empty( $args['unit'] ) ) ? '<span class="wcgs--label wcgs--label-unit">'. $args['units'][0] .'</span>' : '';
          echo '</div>';
